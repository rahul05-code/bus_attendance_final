...
class HomePage extends StatefulWidget {
  const HomePage(
      {super.key}); // Best practice: add const constructor with super.key
...
class _HomePageState extends State<HomePage> {
  ...
  void _loadUser() async {
    ...
    try {
      ...
      final uid =
          user.uid; // Get the unique ID of the current authenticated user
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text("Error loading user data: $e")), // Display the error
        );
      }
      setState(() {
        _isLoading = false; // Hide loading indicator
  ...
  Widget _buildDetailRow(IconData icon, String label, String value) {
    return Padding(
      padding: const EdgeInsets.symmetric(
          vertical: 8.0), // Vertical spacing for each row
      child: Row(
        crossAxisAlignment:
            CrossAxisAlignment.center, // Vertically align icon and text
        children: [
          Icon(icon,
              color: Theme.of(context).primaryColor,
              size: 24), // Icon with primary color
          const SizedBox(width: 15), // Spacing between icon and text
          Expanded(
            // Allows text to take remaining space, preventing overflow
            child: Column(
              crossAxisAlignment:
              ...
              children: [
                ...
                Text(
                  value.isNotEmpty
                      ? value
                      : "N/A", // Display value or "N/A" if empty
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Flexible(
                // Allows phone number to wrap or ellipsis
                child: Text(
                  phone,
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Flexible(
                // Allows email to wrap to multiple lines
                child: Text(
                  email,
                  style: const TextStyle(fontSize: 15, color: Colors.black87),
                ),
            ...
            child: Column(
              ...
              children: [
                ...
                _buildContactRow(
                  ...
                  "+91 8128058421", // Placeholder phone
                  "birjutimbadiya24@gmail.com", // Placeholder email
                ),
                const Divider(
                    height: 20,
                    thickness: 0.5,
                    color: Colors.grey), // Subtle divider between contacts
                _buildContactRow(
                  "Vallabh Bhai",
                  "Bus Management", // Placeholder role
  ...
  void _logout() async {
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text("Logged out successfully!")), // Inform user
        );
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final Color accentRed =
        Colors.red.shade400; // A slightly lighter red for accents
                ...
                children: [
                  CircularProgressIndicator(
                    valueColor: AlwaysStoppedAnimation<Color>(
                        primaryRed), // Your primary red color
          ...
          : _hasError // Conditional rendering for error state
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.error_outline,
                          size: 60, color: accentRed), // Error icon
                      const SizedBox(height: 20),
                      const Text(
                      ...
                      ElevatedButton.icon(
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor:
                              primaryRed, // Use primary red for button
              ...
              : SingleChildScrollView(
                  ...
                  padding: const EdgeInsets.all(
                      20), // Padding around the main content
                  child: Column(
                    crossAxisAlignment:
                    ...
                    children: [
                      ...
                      Card(
                        elevation:
                            6, // Increased elevation for more prominent card
                        shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(
                                15)), // More rounded corners
                        child: Padding(
                          padding: const EdgeInsets.all(
                              25), // Inner padding of the card
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(
                                ...
                                style: TextStyle(
                                  ...
                                  fontWeight: FontWeight.bold,
                                  color:
                                      primaryRed, // Use primary red for title
                                ),
                              ),
                              ...
                              _buildDetailRow(
                                  Icons.calendar_today, "Semester", _sem),
                              _buildDetailRow(
                                  Icons.location_city, "City", _city),
                              _buildDetailRow(
                                  Icons.directions_bus, "Bus No.", _bus),
                      ...
                      const SizedBox(
                          height: 30), // Spacing after the profile card

                      // Row for the icon buttons
                      Center(
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment
                              .spaceEvenly, // Space out the buttons
                          children: [
                              ...
                              children: [
                                IconButton(
                                  icon: Icon(Icons.qr_code_scanner,
                                      size: 70,
                                      color: primaryRed), // Larger QR icon
                                  onPressed: () {
                                    if (_name == "N/A" ||
                                        _phone == "N/A" ||
                                        _field == "N/A" ||
                                      ...
                                      ScaffoldMessenger.of(context)
                                        ...
                                        const SnackBar(
                                            content: Text(
                                                "Please ensure all profile data is loaded before scanning.")),
                                      );
                                      return;
                                    }
                                    Navigator.push(
                                  ...
                                  tooltip:
                                      "Scan Attendance", // Tooltip for accessibility
                                ),
                                const Text(
                                  "Scan Attendance",
                                  style: TextStyle(
                              ...
                              children: [
                                IconButton(
                                  icon: Icon(Icons.person_pin,
                                      size: 70,
                                      color:
                                          primaryRed), // Contact icon (or Icons.perm_contact_calendar, etc.)
